...
function createCommand(model, gltfNode, runtimeNode, context, scene3DOnly) {
  ...
  for (var i = 0; i < length; ++i) {
    ...
    uniformMap = combine(uniformMap, {
      ...
      u_tailorTilesArea: function() {
        if (ExpandBySTC.tailorTilesArea && ExpandBySTC.tailorTilesArea._colorTextures && ExpandBySTC.tailorTilesArea._colorTextures[0]) {
          return ExpandBySTC.tailorTilesArea._colorTextures[0];
        } else {
          return context.defaultTexture;
        }
      },
      u_currModelMatrix: function() {
        return model.modelMatrix;
      },
      u_enableTailorTiles: function() {
        return ExpandBySTC.enableTailorTiles;
      },
      u_enableBrightTiles: function() {
        return ExpandBySTC.enableBrightTiles;
      },
      u_contrast: function(){
        return ExpandBySTC.tailorTiles.contrast;
      },
      u_inverTailorTilesCenterMat: function() {
        return ExpandBySTC.inverTailorTilesCenterMat || new Matrix4();
      },
      u_tailorTilesRect: function() {
        return ExpandBySTC.tailorTilesRect || new Cartesian4();
      },
      bmScaleZNum:function(){
          var num = ExpandBySTC.bmScaleZNum;
          if (!num && num !==0) {
            num = 0;
          }
          return num;
      },
      u_styleBmImg: function(){
          return ExpandBySTC.styleBmImg || context._defaultTexture;
      },
      gltfScaleZNum: function() {
        var num = ExpandBySTC.gltfScaleZNum;
          if (!num && num !==0) {
            num = 0;
          }
        return num;
    ...
    if (loadResources.pendingBufferLoads === 0) {
      if (!loadResources.initialized) {
        ...
        if (!defined(this.gltf.extras.sourceVersion)) {
          var gltf = this.gltf;
          gltf.styleBM = this.styleBM;
          gltf.uvMove = this.uvMove;
          gltf.appear = this.appear;
          gltf.ableCustomLight = gltf.ableCustomLight || this.ableCustomLight;
