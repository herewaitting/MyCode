...
uniform vec4 u_baseMapColor;
uniform bool u_baseMapBlend;
uniform vec4 u_initialColor;
uniform bool u_enableGlobeLight;
...
uniform bool u_enableTailorBright;
uniform bool u_showTailorOnly;
uniform float u_tailorBrightNum;
...
if (u_enableTailorBright) {
    vec4 tlpos = u_inverTailorCenterMat * vec4(v_positionMC,1.0);
    vec2 tuv = (tlpos.xy - u_tailorRect.xy)/ u_tailorRect.zw;
    vec4 tColor = texture2D(u_tailorArea, tuv);
    if (!(tuv.x>=0.0 && tuv.x<=1.0 && tuv.y>=0.0 && tuv.y<=1.0) || (tColor.r<0.5 && tColor.a<0.5)) {
        finalColor *= (1.0 - u_tailorBrightNum);
    }else {
        finalColor *= (1.0 + u_tailorBrightNum);
    }
}
    ...
    if(u_baseMapBlend){
        finalColor = vec4(mix(finalColor.rgb,u_baseMapColor.rgb,u_baseMapColor.a),finalColor.a);
    }
    
    //  float diffuseIntensity = clamp(czm_getLambertDiffuse(czm_extendPointLightPositionEC, normalize(v_normalEC)) * 0.9 + 0.3, 0.0, 1.0);
    //  vec3 pointLightToPot = normalize(v_positionEC.xyz - czm_extendPointLightPositionEC);
    //  vec3 pointLightDir = czm_extendPointLightPositionEC - v_positionEC;
    //  pointLightDir = normalize(pointLightDir);
    //  float nDotSTC = max(dot(pointLightDir,normalize(v_normalEC)), 0.0);
    //  vec3 reflectN = normalize(reflect(-pointLightDir, normalize(normalstc)));
    //  vec3 potToEye = -normalize(v_positionEC);
    //  const float constantAttenuation = 1.0;
    //  const float linearAttenuation = 1.0;
    //  const float quadraticAttenuation = 1.0;
    //  float dis = length(czm_extendPointLightPositionEC - v_positionEC); 
    ...
    //  vec3 diffuseStc = att * czm_extendPointLightColor; 
    //  vec3 jmfsC = 3.0 * att * czm_extendPointLightColor * pow(jmfsDot, 32.0); 
    //  finalColor.rgb += jmfsC + diffuseStc;
    
    // vec3 dx = dFdx(v_positionEC.xyz);
    // vec3 dy = dFdy(v_positionEC.xyz);
    // vec3 normalstc = normalize(cross(dx,dy));
    // if (u_enableGlobeLight) {
    //     czm_geometryInput geometry;
    //     geometry.normal = normalstc;
    //     geometry.metalness = 0.5;
    //     geometry.roughness = 0.5;
    //     geometry.color = finalColor;
    ...
    //     finalColor.rgb += czm_stcPointLight(v_positionEC, geometry, light);
    //     finalColor.rgb = czm_stcSpotLight(finalColor.rgb, normalstc, v_positionEC, 3.0, vec2(0.0));
    // }
