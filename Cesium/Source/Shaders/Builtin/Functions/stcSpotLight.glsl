...
vec3 fresnelSchlick23(vec3 f0, vec3 f90, float VdotH) 
{
    return f0 + (f90 - f0) * pow(clamp(1.0 - VdotH, 0.0, 1.0), 5.0);
}

float smithVisibilityG13(float NdotV, float roughness) 
{
    float k = (roughness + 1.0) * (roughness + 1.0) / 8.0;
    return NdotV / (NdotV * (1.0 - k) + k);
}

float smithVisibilityGGX3(float roughness, float NdotL, float NdotV) 
{
    return smithVisibilityG13(NdotL, roughness) * smithVisibilityG13(NdotV, roughness);
}

float GGX3(float roughness, float NdotH) 
{
...
vec3 czm_stcSpotLight(vec3 positionEC, czm_geometryInput geometry, czm_spotLightInput light) {
    vec3 newColor = vec3(0.0);
    if (czm_extendLightSwitch.z) {
        float dis = length(light.position - positionEC);
        if (dis <= light.dis) {
            vec3 pointl = normalize(light.position - positionEC.xyz);
            float SpotFactor = dot(pointl, light.direction);
            if (SpotFactor >= light.angle) {
                ...
                vec3 pF = fresnelSchlick23(pr0, pr90, pVdotH);
                float pG = smithVisibilityGGX3(alpha, pNdotL, pNdotV);
                float pD = GGX3(alpha, pNdotH);
