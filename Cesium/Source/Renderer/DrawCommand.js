...
DrawCommand.prototype.execute = function (context, passState) {
  if (passState && passState.MirrorEnable) {
    var ableMirror = false;
    if (this.owner && this.owner instanceof SkyBox) {
      ableMirror = true;
    }
    if (this.pass && this.pass == 4) {
      ableMirror = true;
    }
    if (this.owner && this.owner.primitive && this.owner.primitive instanceof Model) {
      ableMirror = true;
    }
    if (!ableMirror) {
      return;
...
var picking = false;
    if (
        ...
        context.uniformState.frameState &&
        context.uniformState.frameState.passes
    ) {
        picking = context.uniformState.frameState.passes.pick;
    }
    if ((this.bloomInside || (this._owner && this._owner.bloomInside)) &&
    !picking && !passState.MirrorEnable) {
      if (ExpandBySTC.resetBloomSize) {
        ExpandBySTC.bloomInside.primitiveBloomFBO && ExpandBySTC.bloomInside.primitiveBloomFBO.destroy();
        ExpandBySTC.bloomInside.primitiveBloomFBO = undefined;
        ExpandBySTC.oldWidth = context.drawingBufferWidth;
        ExpandBySTC.oldHeight = context.drawingBufferHeight;
      }
      this.renderState.depthTest.enabled = true;
      ...
      if (!ExpandBySTC.bloomInside.primitiveBloomFBO) {
        ...
        ExpandBySTC.bloomInside.primitiveBloomFBO = new Framebuffer({
            context: context,
            colorTextures: [tt],
      ...
      if (!ExpandBySTC.bloomInside.primitiveBloomFBOInside) {
        var tt = new Texture({
            context: context,
            width: context.drawingBufferWidth,
            height: context.drawingBufferHeight,
        ...
        ExpandBySTC.bloomInside.primitiveBloomFBOInside = new Framebuffer({
            context: context,
            colorTextures: [tt],
            depthStencilTexture: passState.framebuffer.depthStencilTexture,
            destroyAttachments: false
      ...
      if (this.insideArea || this.owner && this.owner.insideArea) {
        this.framebuffer = ExpandBySTC.bloomInside.primitiveBloomFBOInside;
      } else {
        this.framebuffer = ExpandBySTC.bloomInside.primitiveBloomFBO;
      }
    ...
    if (
        (this.ableBloom ||
          (this._owner && this._owner.ableBloom) || (this._owner && this._owner.primitive && this._owner.primitive.ableBloom) ||
            (this.pass == 4 && ExpandBySTC.bloomTiles.ableBloom)) &&
        !picking && !passState.MirrorEnable
    ) {
        if (ExpandBySTC.resetBloomSize) {
            ExpandBySTC.primitiveBloomFBO &&
                ExpandBySTC.primitiveBloomFBO.destroy();
            ExpandBySTC.tilesBloomFBO && ExpandBySTC.tilesBloomFBO.destroy();
            ExpandBySTC.primitiveBloomFBO = undefined;
            ExpandBySTC.tilesBloomFBO = undefined;
            ExpandBySTC.oldWidth = context.drawingBufferWidth;
            ExpandBySTC.oldHeight = context.drawingBufferHeight;
        }
        this.renderState.depthTest.enabled = true;
        if (!ExpandBySTC.primitiveBloomFBO && this.pass !== 4) {
            var tt = new Texture({
                context: context,
                width: context.drawingBufferWidth,
                height: context.drawingBufferHeight,
            ...
            ExpandBySTC.primitiveBloomFBO = new Framebuffer({
                context: context,
                colorTextures: [tt],
                depthStencilTexture: passState.framebuffer.depthStencilTexture,
                destroyAttachments: false
        ...
        if (!ExpandBySTC.tilesBloomFBO && this.pass === 4) {
            var tt = new Texture({
                context: context,
                width: context.drawingBufferWidth,
                height: context.drawingBufferHeight,
            ...
            ExpandBySTC.tilesBloomFBO = new Framebuffer({
                context: context,
                colorTextures: [tt],
        ...
        this.framebuffer = ExpandBySTC.primitiveBloomFBO;
        if (this.pass === 4) {
            this.framebuffer = ExpandBySTC.tilesBloomFBO;
        }
        context.draw(this, passState);
        this.framebuffer = undefined;
    }
