...
import { CustomHtmlDraw } from "../../../tools/CustomHtmlDraw";
...
export interface IHtmlDrawInfo {
    index: number[];
    uvs: number[];
    dirs: number[];
    VAO: any;
...
export interface IHtmlMergeDrawInfo {
    [key: string]: any;
}

export class CustomHtml extends Point<ILayerStyle> {
    public workerFunName!: string;
    public tool!: CustomHtmlDraw;
    public mergeDraw!: IHtmlMergeDrawInfo;
    public singleDraw!: IHtmlDrawInfo;
    ...
    protected async onStyle<ILayerStyle>(style: ILayerStyle) {
        ...
        this.tool = new CustomHtmlDraw(this.style);
    ...
    protected prepareBgTexture() {
        ...
        for (const con of cons) {
            ...
            img.onload = () => {
                ...
                this.appearance[con].naturalRatio = img.height / img.width;
            };
            img.src = currBgImg;
        }
    }
    ...
    private prepareMerge(option: IWorkerMsg, info: ITextDrawInfo) {
        ...
        // let currMat = trans;
        // tslint:disable-next-line: max-line-length
        const ratio = this.appearance[option.currStyle] && this.appearance[option.currStyle].naturalRatio || this.appearance.default.naturalRatio || 1.0;
        const zxj = new Cesium.Cartesian3(-0.5, -ratio / 2, 0);
        ...
        const zsj = new Cesium.Cartesian3(-0.5, ratio / 2, 0);
        ...
        const ysj = new Cesium.Cartesian3(0.5, ratio / 2, 0);
        ...
        const yxj = new Cesium.Cartesian3(0.5, -ratio / 2, 0);
        ...
        for (const domId of mergeKey) {
            ...
            const command = new (Cesium as any).DrawCommand({
                ...
                uniformMap: {
                    ...
                    billWidth() {
                        return that.appearance[VAO.option.currStyle || "default"].naturalWidth / 2 || 30;
                    },
                    scaleByDistance() {
        ...
        for (const VAO of this.singleDraw.VAO) {
            ...
            const command = new (Cesium as any).DrawCommand({
                ...
                uniformMap: {
                    ...
                    billWidth() {
                        return that.appearance[VAO.option.currStyle || "default"].naturalWidth / 2.0 || 30;
                    },
                    scaleByDistance() {
                        const style = that.appearance[VAO.option.currStyle || "default"];
                        return new Cesium.Cartesian4(style.near, style.scale, style.far, style.ratio);
