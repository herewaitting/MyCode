...
export class WishPoint extends Point<ILayerStyle> {
    public workerFunName!: string;
    private temparr: any;
    private material: any;
    private materialArr!: any[];
    private appearanceArr!: any[];
    private materialNum!: number;
    ...
    protected onStyle<ILayerStyle>(style: ILayerStyle): void {
        ...
        this.material = new LinePrimitiveColorFlowMaterial(newStyle);
        this.materialArr.push(this.material);
        this.appearance = {};
        this.appearance.default = new KVAppearance({
            type: "PolylineMaterial",
            material: this.material,
        });
        this.appearanceArr.push(this.appearance.default);
        for (let i = 0; i < this.materialNum; i++) {
            const material = new LinePrimitiveColorFlowMaterial(newStyle);
            this.materialArr.push(material);
            const app = new KVAppearance({
                type: "PolylineMaterial",
                material: material,
            });
            this.appearanceArr.push(app);
        }
    ...
    protected onData(option: IWorkerMsg): void {
        ...
        if (!points.length && (points as any).x && (points as any).y) {
            ...
            const p2 = adjustCartesina3Height(p11 as any, this.style.height);
            data = [p11, p2];
        } else {
            data = option.dataArr;
        }
        ...
        if (true) {
            ...
            const randomNum = Math.floor(Math.random() * this.materialNum);
            primitive.setAppearance(this.appearanceArr[randomNum]);
    ...
    protected onDestroy(): void {
        if (this.viewer) {
            ...
            // }
            for (let i = 0; i < this.materialNum; i++) {
                this.materialArr[i].material.destroy();
            }
    ...
    private resetMaterial() {
        ...
        for (let i = 0; i < this.materialNum; i++) {
            this.materialArr[i].reset(this.style);
        }
        this.changeBloom(this.style.bloom);
    }
}
