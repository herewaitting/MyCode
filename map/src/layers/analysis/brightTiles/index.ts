...
export class BrightTiles extends Analysis<ILayerStyle> {
    ...
    public setData(oriData: any) {
        ...
        if (oriData instanceof Array) {
            const posArr = oriData.reduce((a, b) => a.concat([b[0], b[1]]), []);
            let data;
            data = Cesium.Cartesian3.fromDegreesArray(posArr);
            if (!this.locatedPos) {
    ...
    protected onHide(): void {
        (Cesium.ExpandBySTC as any).enableBrightTiles = false;
    }
    protected onShow(): void {
        (Cesium.ExpandBySTC as any).enableBrightTiles = true;
    }
