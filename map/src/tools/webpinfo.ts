...
export interface ICCPChunk extends Chunk { // tslint:disable-line
  type: "ICCP";
}
...
export class WebPInfo extends StreamParserWritable {
  ...
  private readonly LOG_TAG = "webpinfo";
  ...
  private readonly FRAME_COUNTABLE_CHUNK_TYPES = new Map<ChunkType, true>([
    ...
    "VP8L",
    "ANMF",
  ].map((t) => [t, true] as [ChunkType, true]));
  ...
  constructor() {
    ...
    thunk.seq([
      ...
      const { isAnimated, isLossless, hasAlpha, width, height } = chunks.reduce((hash: { width: any; height: any; isLossless: boolean; hasAlpha: boolean; isAnimated: boolean; }, c: { type: string; width: any; height: any; alpha: any; }, index: number) => {
      ...
      const summary = {
        ...
        hasAlpha,
        width, height,
        frames: chunks.filter((c: { type: string; }) => this.FRAME_COUNTABLE_CHUNK_TYPES.has(c.type as any)).length,
        chunks: chunks.reduce((hash: { [x: string]: number; }, chunk: { type: string; }) => {
  ...
  public end(...args: any[]) {
    if (args.length && typeof args[args.length - 1] === "function") {
      return super.end(...args.slice(0, args.length - 1), (...cbArgs: any[]) => {
        this.final();
  ...
  private final() {
    if (this.pending) {
      if (this.pending.size !== Infinity) {
        this.pending.done(new Error(`stream is closed before read ${this.pending.size} bytes`));
      }
  ...
  private readRIFFHeader() {
    ...
    return this.readBytes(8)((e: any, buf: any) => {
      if (e) {
        throw e;
      }
  ...
  private readWebPSignature() {
    ...
    return this.readBytes(4)((e: any, buf: any) => {
      if (e) {
        throw e;
      }
  ...
  private readWebPChunk(memo: WebPChunk[] = []): any { // sadly thunk typing is not good
    if (this.offset > this.maxSeekableOffset) {
      this.log("reached max seekable offset (current: %d, max: %d)", this.offset, this.maxSeekableOffset);
    ...
    return this.readBytes(8)((e: any, buf: any) => {
      ...
      const chunkSize = (chunkPayloadSize % 2 === 0 ? chunkPayloadSize : chunkPayloadSize + 1) + 8;

      this.log("detected chunk (%s) payload size: %d, offset: %d",
      ...
      if (Buffer.compare(BUF_VP8, bufChunkType) === 0) {
        return this.readVP8Bitstream()((err: any, bitstream: any): VP8Chunk => {
          if (err) {
            throw err;
          }
        ...
        return this.readVP8LBitstream()((err: any, bitstream: any): VP8LChunk => {
          if (err) {
            throw err;
          }
        ...
        return this.readVP8XBitstream()((err: any, bitstream: any): VP8XChunk => {
          if (err) {
            throw err;
          }
        ...
        return this.readANIMBitstream()((err: any, bitstream: any): ANIMChunk => {
          if (e) {
            throw e;
          }
        ...
        return this.readANMFBitstream()((err: any, bitstream: any): ANMFChunk => {
          if (e) {
            throw e;
          }
        ...
        return this.readALPHBitstream()((err: any, bitstream: any): ALPHChunk => {
          if (e) {
            throw e;
          }
          ...
          return {
            ...
            offset: chunkOffset,
            size: chunkSize,
            preprocessing: bitstream.preprocessing === 0 ?
              ALPHPreProcessing.NO_PRE_PROCESSING :
              ALPHPreProcessing.LEVEL_REDUCTION,
            filter: (() => {
              switch (bitstream.filtering) {
                case ALPHFilteringMethod.HORIZONTAL: return ALPHFilteringMethod.HORIZONTAL;
                case ALPHFilteringMethod.VERTICAL: return ALPHFilteringMethod.VERTICAL;
                case ALPHFilteringMethod.GRADIENT: return ALPHFilteringMethod.HORIZONTAL;
                default: return ALPHFilteringMethod.NONE;
              }
            })(),
            compress: bitstream.compression === 0 ?
        ...
        throw new Error(`Unsupported WebP chunk type ${chunkType} (0x${chunkTypeHex})`);
      }
    })((e: any, chunk: any) => {
      if (e) {
        throw e;
  ...
  private readVP8Bitstream() {
    ...
    return this.readBytes(3)((e: any, bufFrameHeader: any) => {
      if (e) {
        throw e;
      }

      const frameTag = bufFrameHeader.readUInt8(0) |
      ...
      this.log("Parsed VP8 Bitstream Header: keyframe: %s, version: %s, showFrame: %s, firstPartSize: %s",
        keyframe,
        version,
        showFrame,
        firstPartSize,
      ...
      if (keyframe === 1) {
        throw new Error("expected VP8 bitstream has keyframe, but couldn't be found");
      }

      // If given frame is Keyframe (Intraframe), It has 7 bytes data for additional information
      return this.readBytes(7)((err: any, bufKeyframeTag: any) => {
        if (err) {
          throw err;
        }

        const hasValidStartCode = Buffer.compare(
        ...
        if (!hasValidStartCode) {
          ...
          throw new Error("expected VP8 intraframe start code, but couldn't be found");
        }
        ...
        this.log("width: %d, horizontalScale: %d, height: %d, verticalScale: %d",
          width,
          horizontalScale,
          height,
          verticalScale,
  ...
  private readVP8LBitstream() {
    ...
    return this.readBytes(1)((e: any, bufVP8LSignature: any) => {
      if (e) {
        throw e;
      }
    ...
    })((e: any, bufBitstreamHead: any): VP8LBitstream => {
      if (e) {
        throw e;
      }
      ...
      if (version !== 0) {
        throw new Error("expected VP8L bitstream version to 0, but got non-zero value");
      }

      this.log("width: %d, height: %d, alpha: %s, version: %d", width, height, alpha, version);
  ...
  private readVP8XBitstream() {
    ...
    return this.readBytes(10)((e: any, buf: any): VP8XBitstream => {
      if (e) {
        throw e;
      }
      ...
      this.log("rsv1: %d, icc: %d, alpha: %d, exif: %d, xmp: %d, animation: %d, rsv2: %d",
        rsv1,
        icc,
        alpha,
        exif,
      ...
      const canvasWidth = (buf.readUInt8(4) |
        buf.readUInt8(5) << 8 |
        buf.readUInt8(6) << 16) + 1;

      const canvasHeight = (buf.readUInt8(7) |
        buf.readUInt8(8) << 8 |
        buf.readUInt8(9) << 16) + 1;

      this.log("canvas width: %d, canvas height: %d", canvasWidth, canvasHeight);

      return {
        rsv1, icc, alpha, exif,
        xmp, animation, rsv2, rsv3,
  ...
  private readANIMBitstream() {
    ...
    // 16 bits - Loop Count
    return this.readBytes(6)((e: any, buf: any): ANIMBitstream => {
      if (e) {
        throw e;
      }
  ...
  private readANMFBitstream() {
    ...
    return this.readBytes(16)((e: any, buf: any): ANMFBitstream => {
      if (e) {
        throw e;
      }

      const frameX = buf.readUInt8(0) |
        buf.readUInt8(1) << 8 |
        buf.readUInt8(2) << 16;

      const frameY = buf.readUInt8(3) |
        buf.readUInt8(4) |
        buf.readUInt8(5);

      const frameWidth = (buf.readUInt8(6) |
        buf.readUInt8(7) |
        buf.readUInt8(8)) + 1;

      const frameHeight = (buf.readUInt8(9) |
        buf.readUInt8(10) |
        buf.readUInt8(11)) + 1;
  ...
  private readALPHBitstream() {
    ...
    return this.readBytes(1)((e: any, buf: any): ALPHBitstream => {
      if (e) {
        throw e;
      }
